    ...
    <Section title="FEATURED TAGS" to="/tags">
      <div className="featured-tags">
        {featuredTags.map((item, index) => (
          <Tag key={`${item}-${index}`} to={`/tags#${item}`}>
            {item}
          </Tag>
        ))}
      </div>
    </Section>
    <Section title="ABOUT ME" to="/about">
      <div className="short-about">
        <Link to="/about">
          <img
            className="short-about__avatar rounded"
            ...
            <li key={`${item}-${index}`} className="list-inline-item">
              <a className="sns__link" target="_blank" href={item.to}>
                <span className="fa-stack fa-lg">
                  ...
                  {item.icon ? (
                    <i
                      className={`fa fa-${item.icon} fa-stack-1x fa-inverse`}
                    />
                  ) : (
    ...
    <Section title="FRIENDS">
      <ul className="friend-links list-inline">
        {friends.map((item, index) => (
          <li key={`${item}-${index}`} className="list-inline-item">
            <a className="friend-links__link" href={item.to} target="_blank">
              {item.name}
            </a>
          </li>
        ))}
