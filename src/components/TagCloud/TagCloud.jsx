...
const colorIncrement = function(range) {
  return toRGB(colorOpts.end).map(
    (n, i) => (n - toRGB(colorOpts.start)[i]) / range
  )
}
...
const TagCloud = props => {
  ...
  const Cloud = props.dataSource.map((item, index) => {
    ...
    return (
      <Tags key={`${item}-${index}`} custom-color={color} to={`/tags/#${item}`}>
        {item}
      </Tags>
    )
  })

  return <div className="tagCloud">{Cloud}</div>
}
